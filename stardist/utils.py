...
from stardist.matching import matching_dataset, _check_label_array
...
def optimize_threshold(Y, Yhat, model, nms_thresh, iou_threshs, measure='accuracy', 
                     bracket = None, tol=1e-2, maxiter=20, verbose=1):
    ...
    with tqdm(total=maxiter, disable=(verbose!=1), desc="NMS threshold = %g" % nms_thresh) as progress:
        ...
        def fn(thr):
            ...
            if value is None:
                Y_instances = [model._instances_from_prediction(y.shape, *prob_dist, 
                prob_thresh=prob_thresh, nms_thresh=nms_thresh)[0] for y,prob_dist in zip(Y,Yhat)]
